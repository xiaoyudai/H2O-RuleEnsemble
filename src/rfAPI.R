...
DefaultRFContext <- function()
{
  ...
  rf.ctxt$memory.par <- 0.1
  rf.ctxt$conv.thr   <- 1.0e-3
  rf.ctxt$mem.tree.store <- 10000000
  rf.ctxt$mem.cat.store  <- 1000000
  rf.ctxt$quiet <- TRUE
  # new for h2o
  rf.ctxt$regularization.alpha <- 1
  rf.ctxt$regularization.lambda <- NULL
  rf.ctxt$data.trim.quantile <- 0.025
  rf.ctxt$pd.sample.size <- 500
  rf.ctxt$qntl <- 0.025
  rf.ctxt$max.var.vals <- 40
...
InitRFContext <- function(model.conf.fname, data.conf)
{
  ...
  kKnownParamNames <- c("rf.platform", "rf.working.dir", "rf.export.dir", "task", "model.type",
                        ...
                        "pd.sample.size", "qntl", "max.var.vals",  
                        "regularization.alpha", "regularization.lambda", "ip", "port", "nthreads")
  ...
  # new for H2O
  if ("regularization.alpha" %in% names(conf)) {
    rf.ctxt$regularization.alpha <- as.numeric(conf$regularization.alpha)
  }
  
  if ("regularization.lambda" %in% names(conf)) {
    rf.ctxt$regularization.lambda <- as.numeric(conf$regularization.lambda)
  }
  
  if ("data.trim.quantile" %in% names(conf)) {
    rf.ctxt$data.trim.quantile <- as.numeric(conf$data.trim.quantile)
  }
  
  if ("pd.sample.size" %in% names(conf)) {
    rf.ctxt$pd.sample.size <- as.numeric(conf$pd.sample.size)
  }
  
  if ("qntl" %in% names(conf)) {
    rf.ctxt$qntl <- as.numeric(conf$qntl)
  }
  
  if ("max.var.vals" %in% names(conf)) {
    rf.ctxt$max.var.vals <- as.numeric(conf$max.var.vals)
  }
