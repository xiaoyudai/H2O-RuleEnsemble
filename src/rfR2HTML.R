...
WriteHTML <- function(rego_Rules, conf, model.path, out.path = model.path, rfmod, y, y.hat)
{
  ...
  #    rfmodel : glm on rules and linear terms (a H2O glm model object, usually LASSO)
  #               y: response vector
  #           y.hat: prediction on response
  #
  # Returns:
  #     None.
  kPlotWidth <- 620
  ...
  if(rfmod@allparameters$family == "binomial"){
    # classification
    mse = h2o.mse(rfmod)
    r2 = h2o.r2(rfmod)
    logloss = h2o.logloss (rfmod)
    mpce = h2o.mean_per_class_error(rfmod)
    auc = h2o.auc(rfmod)
    nd = h2o.null_deviance(rfmod)
    rd = h2o.residual_deviance(rfmod)
    aic = h2o.aic(rfmod)
    ...
    mse = h2o.mse(rfmod)
    r2 = h2o.r2(rfmod)
    mrd = h2o.mean_residual_deviance(rfmod)
    nd = h2o.null_deviance(rfmod)
    ndf = h2o.null_dof(rfmod)
    rd = h2o.residual_deviance(rfmod)
    rdf = h2o.residual_dof(rfmod)
    aic = h2o.aic(rfmod)
  ...
  # Write out model metric
  HTML(metric.df, caption = "Model Performance", row.names = F, innerBorder = 1)
#   
#   # test error and ROC
