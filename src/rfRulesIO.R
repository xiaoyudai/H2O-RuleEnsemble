...
kSplitTypeDiffer <- "differ"
kSplitTypeIsNAorLess <- "isNAorLess"
kSplitTypeIsNA <- "isNA"
kMinusInf             <- -9.9e+35
kPlusInf              <- 9.9e+35
kMissing              <- 9e+30
...
ReadRules <- function(file)
{
  ...
  ParseSplitRule <- function(file, ruleLgth) {
    ...
    kDiff.split.id1.str <- "equal"
    kDiff.split.id2.str <- "un"
    ...
    while (length(res$token) > 0 && res$token != "Rule") {
      ...
      } else if (res$token == kDiff.split.id1.str || res$token == kDiff.split.id2.str) {
        # "Differ" split... need point
        if (res$token == kDiff.split.id2.str) {
          differ.cond <- kDiff.split.id2.str
          res <- NextRuleFitToken(file)
        } else {
          differ.cond <- kDiff.split.id1.str
        }
        res <- NextRuleFitToken(file)
        differ.pt <- as.numeric(res$token)
        split <- list(type=kSplitTypeDiffer, var = splitVarName, cond = differ.cond, pt = differ.pt)
      } else {
    ...
    # sometimes we do splits on the same node (as in the tricky type 2 condition in H2O POJO)
...
SplitRule2Char <- function(rule, x.levels, x.levels.lowcount)
{
  ...
  DifferSplit2Char <- function(split) {
    # Turns a 'differ' split into a human 'readable' string
    #
    # Args:
    #       split : list of the form <type="differ", var, cond, point>
    #
    # Returns:
    #      A character vector with one of these strings: "var == point",
    #      or "var != point"
    if(split$cond == "equal"){
      str <- paste(split$var, "==", split$point)
    } else if(split$cond == "unequal"){
      str <- paste(split$var, "!=", split$point)
    } else {
      error(logger, paste("DifferSplit2Char: don't know how to print split: ", split))
    }
    
    return(str)
  }
  
  isNAorLessSplit2Char <- function(split) {
    # Turns a 'isNAorLess' split into a human 'readable' string
    #
    # Args:
    #       split : list of the form <'isNAorLess', var, min, max>
    #
    # Returns:
    #     
    str <- paste(split$var, 'is NULL or', split$var, '<', split$max)
    return(str)
  }
  
  isNASplit2Char <- function(split) {
    # Turns a 'isNAorLess' split into a human 'readable' string
    #
    # Args:
    #       split : list of the form <'isNA', var, isNA|notNA>
    #
    # Returns:
    
    if(split$Is == 'isNA'){
      str <- paste(split$var, 'is NULL')
    } else {
      str <- paste(split$var, 'is not NULL')
    }
    return(str)
  }
  ...
  for (iSplit in 1:nSplits) {
    ...
    } else if (split$type == kSplitTypeDiffer) {
      if (iSplit == 1) {
        splitStr <- DifferSplit2Char(split)
      } else {
        splitStr <- paste(splitStr, kSplit.and.str, DifferSplit2Char(split))
      }
    } else if (split$type == kSplitTypeIsNAorLess) {
      if (iSplit == 1) {
        splitStr <- isNAorLessSplit2Char(split)
      } else {
        splitStr <- paste(splitStr, kSplit.and.str, isNAorLessSplit2Char(split))
      }
    } else if (split$type == kSplitTypeIsNA) {
      if (iSplit == 1) {
        splitStr <- isNASplit2Char(split)
      } else {
        splitStr <- paste(splitStr, kSplit.and.str, isNASplit2Char(split))
      }
    }
